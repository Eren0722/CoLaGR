...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data_root", default="./datasets", help="Root directory that contains the prepared dataset")
...
def load_llm_from_local_path(path: str, device: torch.device):
    ...
    tokenizer.add_special_tokens({
        'additional_special_tokens': ['[UserRep]', '[ItemEmb]', '[UserOut]', '[ItemOut]']
    })
...
def load_text_dict(dataset: str, data_root: str) -> Dict[str, Dict]:
    candidates = [
        os.path.join(data_root, dataset, "text_name_dict.json.gz"),
    ]
    meta_path = next((path for path in candidates if os.path.exists(path)), candidates[0])
    if not os.path.exists(meta_path):
        raise FileNotFoundError(
            f"Missing metadata file: {meta_path}. Run dataset preprocessing first."
        )
...
def main():
    ...
    print(f"Saved sequence keys: {seq_keys_path} ({len(seq_keys_to_int)} entries)")

    tokenizer, model = load_llm(args, device)

    item_prompts = build_item_prompts(itemnum, text_dict)
    print(f"Generating item semantic embeddings for {len(item_prompts) - 1} items")
    ...
    print(f"Saved item semantic embeddings: {tuple(item_emb.shape)} -> {item_emb_path}")

    seq_prompts = build_sequence_prompts(seq_keys_to_int, text_dict, args.history_trunc)
    print(f"Generating user semantic embeddings for {len(seq_prompts)} sequences")
    ...
    print(f"Saved user semantic embeddings: {tuple(seq_emb.shape)} -> {user_emb_path}")

    if not args.skip_neighbors:
        print(f"Computing Top-{args.neighbor_k} semantic neighbors")
        ...
        print(f"Saved semantic neighbor indices -> {output_dir}")

    print("Semantic asset generation completed.")
