...
FULL_PREFIX=${FULL_PREFIX:-electronics_sa_teacher}
WO_USER_PREFIX=${WO_USER_PREFIX:-electronics_wo_usercl_teacher}
WO_ITEM_PREFIX=${WO_ITEM_PREFIX:-electronics_wo_itemcl_teacher}

FULL_SI_SAVE=${FULL_SI_SAVE:-electronics_si_l40s}
PURE_SI_SAVE=${PURE_SI_SAVE:-electronics_pure_sasrec_si_l40s}
WO_USER_SI_SAVE=${WO_USER_SI_SAVE:-electronics_wo_usercl_si_l40s}
WO_ITEM_SI_SAVE=${WO_ITEM_SI_SAVE:-electronics_wo_itemcl_si_l40s}
WO_MATCH_SI_SAVE=${WO_MATCH_SI_SAVE:-electronics_wo_match_si_l40s}
...
preprocess_raw_5core("Electronics")
PY
    local status=$?
...
prepare_assets() {
  ...
  PYTHONPATH=$(pwd) CUDA_VISIBLE_DEVICES=${TRAIN_DEVICE} python SA/generate_assets.py \
    --dataset ${DATASET} \
    --data_root ./SeqRec \
...
train_teacher() {
  ...
  CUDA_VISIBLE_DEVICES=${TRAIN_DEVICE} PYTHONPATH=$(pwd) python -m SeqRec.sasrec.train_sa \
    --dataset ${DATASET} \
    --data_root ./SeqRec \
...
run_si() {
  ...
  CUDA_VISIBLE_DEVICES=${TRAIN_DEVICE} python main.py --train \
    --device 0 \
    --recsys sasrec \
...
run_si "full" "${FULL_CKPT}" "${FULL_SI_SAVE}" 1.0 || exit 1

echo "===== [3/6] pure SASRec -> SI ====="
train_pure_sasrec || exit 1
run_si "pure_sasrec" "${PURE_CKPT}" "${PURE_SI_SAVE}" 1.0 || exit 1

echo "===== [4/6] w/o user-cl -> SI ====="
train_teacher "${WO_USER_PREFIX}" 0.0 0.1 || exit 1
run_si "wo_usercl" "${WO_USER_CKPT}" "${WO_USER_SI_SAVE}" 1.0 || exit 1

echo "===== [5/6] w/o item-cl -> SI ====="
train_teacher "${WO_ITEM_PREFIX}" 0.1 0.0 || exit 1
run_si "wo_itemcl" "${WO_ITEM_CKPT}" "${WO_ITEM_SI_SAVE}" 1.0 || exit 1

echo "===== [6/6] w/o match -> SI ====="
run_si "wo_match" "${FULL_CKPT}" "${WO_MATCH_SI_SAVE}" 0.0 || exit 1
