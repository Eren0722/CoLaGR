...
rm -rf ./models/${DATASET}/${SI_SAVE_DIR}

cd "${ROOT_DIR}/SeqRec/sasrec"
python - <<'PY'
...
PYTHONPATH=$(pwd) CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_addr=${MASTER_ADDR} --master_port=29721 \
...
CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_addr=${MASTER_ADDR} --master_port=29722 \
