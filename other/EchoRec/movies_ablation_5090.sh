...
FULL_SI_SAVE=${FULL_SI_SAVE:-movies_si_5090}
PURE_SI_SAVE=${PURE_SI_SAVE:-movies_pure_sasrec_si_5090}
WO_USER_SI_SAVE=${WO_USER_SI_SAVE:-movies_wo_usercl_si_5090}
WO_ITEM_SI_SAVE=${WO_ITEM_SI_SAVE:-movies_wo_itemcl_si_5090}
WO_MATCH_SI_SAVE=${WO_MATCH_SI_SAVE:-movies_wo_match_si_5090}
...
echo "===== [2/6] full SA -> SI ====="
train_teacher "${FULL_PREFIX}" 0.1 0.1 29831 || exit 1
run_si "full" "${FULL_CKPT}" "${FULL_SI_SAVE}" 1.0 29841 || exit 1

echo "===== [3/6] pure SASRec -> SI ====="
train_pure_sasrec || exit 1
run_si "pure_sasrec" "${PURE_CKPT}" "${PURE_SI_SAVE}" 1.0 29842 || exit 1

echo "===== [4/6] w/o user-cl -> SI ====="
train_teacher "${WO_USER_PREFIX}" 0.0 0.1 29832 || exit 1
run_si "wo_usercl" "${WO_USER_CKPT}" "${WO_USER_SI_SAVE}" 1.0 29843 || exit 1

echo "===== [5/6] w/o item-cl -> SI ====="
train_teacher "${WO_ITEM_PREFIX}" 0.1 0.0 29833 || exit 1
run_si "wo_itemcl" "${WO_ITEM_CKPT}" "${WO_ITEM_SI_SAVE}" 1.0 29844 || exit 1

echo "===== [6/6] w/o match -> SI ====="
run_si "wo_match" "${FULL_CKPT}" "${WO_MATCH_SI_SAVE}" 0.0 29845 || exit 1

echo "===== done ====="
echo "candidate_num=${CAND_NUM}"
