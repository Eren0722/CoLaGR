...
FULL_PREFIX=${FULL_PREFIX:-movies_gru4rec_full_teacher}
WO_SACP_PREFIX=${WO_SACP_PREFIX:-movies_gru4rec_wo_sacp_teacher}

FULL_SI_SAVE=${FULL_SI_SAVE:-movies_gru4rec_full_si_5090}
WO_SACP_SI_SAVE=${WO_SACP_SI_SAVE:-movies_gru4rec_wo_sacp_si_5090}
...
train_teacher "${FULL_PREFIX}" 0.1 0.1 29871 || exit 1
run_si "full" "${FULL_CKPT}" "${FULL_SI_SAVE}" 29881 || exit 1

echo "===== [3/4] w/o SACP: GRU4Rec rec-only -> SI ====="
train_teacher "${WO_SACP_PREFIX}" 0.0 0.0 29872 || exit 1
run_si "wo_sacp" "${WO_SACP_CKPT}" "${WO_SACP_SI_SAVE}" 29882 || exit 1
