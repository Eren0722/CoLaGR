from minigpt4.models.rec_base_models import MatrixFactorization, LightGCN, SASRec, GSASRec
...
def run_a_trail(train_config, data_dir='', log_file=None, save_mode=False,save_file=None,need_train=True,warm_or_cold=None):
    ...
    print("gsasrec_config:\n", gsasrec_config)
    gsasrec_config = omegaconf.OmegaConf.create(gsasrec_config)

    test_data_loader = DataLoader(test_data, batch_size = train_config['batch_size'], shuffle=False)

    model = GSASRec(gsasrec_config).cuda()
...
if __name__=='__main__':
    ...
    parser.add_argument("--save_path", type=str, default="/data/yuqihang/result/CoLLM/checkpoints/gsasrec/")
