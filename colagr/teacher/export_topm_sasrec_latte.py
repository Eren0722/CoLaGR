...
def score_topm_batch(
    ...
    with torch.no_grad():
        ...
        msp_values = None
        if msp_item_embs is not None:
            # Official MemGen-GR confidence: max softmax probability over the
            # complete item vocabulary, not over the exported Top-M list.
            full_logits = final_feats.matmul(msp_item_embs.T)
            msp_values = torch.softmax(full_logits, dim=-1).amax(dim=-1)
    ...
    for row, example in enumerate(examples):
        ...
        if msp_values is not None:
            record['confidence_msp'] = float(msp_values[row].item())
...
def main():
    ...
    parser.add_argument(
        '--export_msp', type=parse_bool, default=False,
        help='Also export full-vocabulary MSP for Adaptive Ensemble evaluation.',
    )
    parser.add_argument(
        '--numeric_item_ids', type=parse_bool, default=False,
        help='Export target/top items as Latte numeric IDs for cross-model alignment.',
    )
    ...
    if args.export_msp and args.legacy_per_sample:
        raise ValueError('--export_msp requires batched scoring; disable --legacy_per_sample.')
    ...
    if not args.legacy_per_sample:
        ...
        if args.export_msp:
            all_item_ids = list(range(1, dataset.n_items))
            msp_item_embs = get_item_embedding_matrix(model, all_item_ids, device)
        print(
    ...
    for split in [part.strip() for part in args.splits.split(',') if part.strip()]:
        ...
        if args.numeric_item_ids:
            for record in records:
                record['target_item'] = int(dataset.item2id[record['target_item']])
                record['top_items'] = [
                    int(dataset.item2id[item]) for item in record['top_items']
                ]
