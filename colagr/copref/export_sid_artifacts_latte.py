...
def main():
    ...
    for level in range(tokenizer.n_digit):
        ...
        assert not torch.isin(level_tensor, torch.tensor([0, tokenizer.eos_token] + tokenizer.copref_token_ids)).any()
    ...
    json.dump({
        ...
        'copref_token_ids': [int(x) for x in tokenizer.copref_token_ids],
