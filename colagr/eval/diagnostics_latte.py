...
def load_copref(path, limit):
    ...
    if isinstance(data, dict) and data.get('format') == 'colagr_copref_prefix_tensor_v2':
...
def main():
    ...
    if isinstance(records, dict) and records.get('format') == 'colagr_copref_prefix_tensor_v2':
