...
def main():
    ...
    for sample_id, example in enumerate(tqdm(splits[args.split], desc='Sweep fusion')):
        ...
        base_rank_scores = {
            item: 1.0 / (args.rrf_constant + rank)
        ...
        for history_length, memory_k, alpha in configs:
            ...
            candidates = base_items + [
                item for item in memory_items if item not in base_rank_scores
            ...
            def fusion_score(item):
                ...
                if args.fusion_mode == 'rrf':
                    ...
                    return base_rank_scores.get(item, 0.0) + alpha * memory_term
                return (
                    base_rank_scores.get(item, 0.0)
            ...
            if args.record_output is not None and len(configs) == 1:
                positive_scores = [
    ...
    if args.record_output is not None:
        ...
        torch.save(user_records, args.record_output)
