...
class Pipeline:
    def __init__(
        ...
        with self.accelerator.main_process_first():
            ...
            if checkpoint_path:
                ...
                allow_extra = bool(self.config.get('allow_checkpoint_extra_keys', False))
                self.model.load_state_dict(checkpoint, strict=not allow_extra)
            ...
            if self.config.get('lareason_e_init_checkpoint_path'):
                lareason_e_path = self.config['lareason_e_init_checkpoint_path']
                self.model.load_lareason_e_state_dict(
                    torch.load(lareason_e_path, map_location=self.config['device'])
                )
                self.log(f'Loaded LaReason-E reasoner parameters from {lareason_e_path}')
            if self.config.get('latempo_init_checkpoint_path'):
                latempo_path = self.config['latempo_init_checkpoint_path']
                self.model.load_latempo_state_dict(
                    torch.load(latempo_path, map_location=self.config['device'])
                )
                self.log(f'Loaded LaTempo parameters from {latempo_path}')
            if hasattr(self.model, 'apply_post_checkpoint_initialization'):
                init_report = self.model.apply_post_checkpoint_initialization()
                if init_report:
                    self.log(init_report)
        self.log(self.model)
    ...
    def run(self, skip_end_training=False):
        ...
        if self.config.get('skip_test_after_training', False):
            checkpoint = self.trainer.saved_model_ckpt
            self.trainer.end(destroy_process_group=not skip_end_training)
            return {'best_epoch': best_epoch, 'best_val_score': best_val_score,
                    'checkpoint': checkpoint, 'test_results': None}
