...
class Trainer:
    ...
    def fit(self, train_dataloader, val_dataloader):
        ...
        if tracker == 'tensorboard':
            ...
            # Use custom wandb_run_name if provided, otherwise use default file name
    ...
    def end(self, destroy_process_group=True):
        """
        Ends the training process and releases any used resources.
        
        Args:
            destroy_process_group: If True, calls accelerator.end_training() which
                destroys the process group. If False, only finishes the trackers
                without destroying the process group (useful for hyper-parameter tuning).
        """
        if destroy_process_group:
            self.accelerator.end_training()
        else:
            # Only finish trackers without destroying process group
            for tracker in self.accelerator.trackers:
                tracker.finish()
            # Clear trackers so they can be re-initialized
