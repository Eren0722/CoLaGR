...
class AmazonReviews2023(AbstractDataset):
    ...
    def _extract_meta_multi_group_sentences(self, meta_dataset):
        """
        Extract metadata as multiple sentence groups for multi-group VQ.
        
        Default grouping:
            - Group 0: title
            - Group 1: categories
            - Group 2: features + description
        
        Returns:
            dict: A dictionary mapping item IDs to a list of sentence groups.
                  e.g., {'item1': ['title text', 'categories text', 'features + description text']}
        """
        # Feature groups: each group will be concatenated into one sentence
        # This creates 3 groups by default: title, categories, features+description
        feature_groups = self.config.get('meta_feature_groups', [
            ['parent_asin'],
            ['title'],
            ['categories', 'features', 'description']
        ])
        
        self.log(f'[DATASET] Extracting multi-group metadata with {len(feature_groups)} groups: {feature_groups}')
        ...
        for example in meta_dataset:
            ...
            for group in feature_groups:
                ...
                for feature in group:
                    group_text += self._feature_process(example[feature])
                group_sentences.append(group_text.strip())
            item2meta[parent_asin] = group_sentences
    ...
    def _process_meta(self, output_path: str):
        ...
        elif process_mode == 'sentence_multi_group':
            item2meta = self._extract_meta_multi_group_sentences(meta_dataset=meta_dataset)
        else:
    ...
    def _build_user_item_timestamp(self, datasets):
        user_item_time = collections.defaultdict(dict)
        for split in ['train', 'valid', 'test']:
            dataset = datasets[split]
            for user, parent_asin, timestamp in zip(
                dataset['user_id'],
                dataset['parent_asin'],
                dataset['timestamp'],
            ):
                try:
                    ...
                    continue
                existing = user_item_time[user].get(parent_asin)
                if existing is None or cur_time < existing:
                    user_item_time[user][parent_asin] = cur_time
        return user_item_time

    def _infer_time_seq(self, user, items, target_time, user_item_time):
        target_time = int(target_time)
        times = [user_item_time.get(user, {}).get(item) for item in items[:-1]]
        times.append(target_time)

        # The public benchmark keeps each row's target timestamp, but the first
        # historical item can be absent after non-history filtering. Fill missing
        # values monotonically so time-gap features stay well-defined without
        # changing the original chronological item sequence.
        next_known = target_time
        for idx in range(len(times) - 2, -1, -1):
            if times[idx] is None:
                times[idx] = max(next_known - 1, 0)
            next_known = int(times[idx])
        last = 0
        for idx, value in enumerate(times):
            value = int(value)
            if idx > 0 and value < last:
                value = last
            times[idx] = value
            last = value
        return times
    ...
    def _timestamp_split(self, datasets):
        user_item_time = self._build_user_item_timestamp(datasets)
        split_data = {}
        for split in ['train', 'valid', 'test']:
            split_data[split] = {'user': [], 'item_seq': [], 'time_seq': []}
            for user, parent_asin, history, timestamp in zip(
                datasets[split]['user_id'],
                datasets[split]['parent_asin'],
                datasets[split]['history'],
                datasets[split]['timestamp'],
            ):
                items_in_history = history.split(' ')
                items_in_history.append(parent_asin)
                time_seq = self._infer_time_seq(
                    user,
                    items_in_history,
                    timestamp,
                    user_item_time,
                ...
                split_data[split]['time_seq'].append(time_seq)
        return {
            'val' if k == 'valid' else k: Dataset.from_dict(v)
            for k, v in split_data.items()
        }

    def _target_time_for_sequence(self, user, items, user_item_time):
        user_times = user_item_time.get(user, {})
        for item in reversed(items):
            if item in user_times:
                return user_times[item]
        return 0

    def _attach_time_seq_to_split_data(self, split_data):
        user_item_time = getattr(self, '_latempo_user_item_time', None)
        if user_item_time is None:
            raise ValueError(
                'LaTempo requires raw review timestamps, but they were not '
                'loaded. Rebuild the dataset with use_latempo=true.'
            )

        output = {}
        for split, dataset in split_data.items():
            if 'time_seq' in dataset.column_names:
                output[split] = dataset
                continue

            columns = {column: list(dataset[column]) for column in dataset.column_names}
            columns['time_seq'] = []
            for user, item_seq in zip(columns['user'], columns['item_seq']):
                target_time = self._target_time_for_sequence(user, item_seq, user_item_time)
                columns['time_seq'].append(
                    self._infer_time_seq(user, item_seq, target_time, user_item_time)
                )
            output[split] = Dataset.from_dict(columns)
        return output

    def split(self):
        split_data = super().split()
        if self.config.get('use_latempo', False) and self.config['split'] != 'timestamp':
            self.split_data = self._attach_time_seq_to_split_data(split_data)
    ...
    def _download_and_process_raw(self):
        ...
        # The original Latte logic only checked whether "processed/" exists and then
        # skipped raw dataset loading entirely. That breaks if a previous run created
        # the directory but did not finish writing all processed artifacts.
        # We only skip dataset loading when the processed cache is actually complete.
        processed_cache_complete = (
            os.path.exists(processed_data_path)
            and os.path.exists(id_mapping_file)
            and os.path.exists(seq_file)
            and (
                self.config['metadata'] == 'none'
                or os.path.exists(meta_file)
            )
        )

        need_raw_sequence_times = bool(self.config.get('use_latempo', False))
        if processed_cache_complete and not need_raw_sequence_times:
            datasets = None
        else:
        ...
        if need_raw_sequence_times:
            if datasets is None:
                raise ValueError(
                    'LaTempo requires raw review timestamps, but the raw dataset '
                    'was not loaded. Set use_latempo=false or rebuild with raw data.'
                )
            self._latempo_user_item_time = self._build_user_item_timestamp(datasets)

        if self.config['split'] == 'timestamp':
            if datasets is None:
                raise ValueError(
                    'Timestamp split requires raw review timestamps, but the raw '
