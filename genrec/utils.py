...
def init_logger(config: dict):
    ...
    # Log command line arguments and config at the start
...
def get_total_steps(config, train_dataloader):
    ...
    """
    if config.get('num_train_epochs', None) is not None:
        config['epochs'] = int(config['num_train_epochs'])
...
def config_for_log(config: dict) -> dict:
    ...
    for k, v in config.items():
        if isinstance(v, (list, tuple, dict)) or v is None:
            config[k] = str(v)
    return config
