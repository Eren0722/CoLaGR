...
class CoLaGRTokenizer(PSIDTokenizer):
    ...
    Labels remain [sid_1, ..., sid_m, eos]. CoPref and CoReason tokens are
    appended after eos in the vocabulary and are only used by decoder internals.
    """
    ...
    def __init__(self, config, dataset):
        ...
        coreason_offset = copref_offset + self.n_digit
        self.coreason_token_ids = [
            coreason_offset + level for level in range(self.n_digit)
        ]
    ...
    def vocab_size(self) -> int:
        return self.psid_eos_token + 1 + 2 * self.n_digit
