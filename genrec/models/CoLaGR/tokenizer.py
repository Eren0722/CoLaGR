...
from genrec.models.CoLaGR.sid_tokenizer import SemanticIDTokenizer


class CoLaGRTokenizer(SemanticIDTokenizer):
    """
    CoLaGR tokenizer using conflict-free semantic IDs.

    Labels remain [sid_1, ..., sid_m, eos]. CoPref tokens are appended after
    eos and are only used by decoder internals.
    """

    def __init__(self, config, dataset):
        super(CoLaGRTokenizer, self).__init__(config, dataset)
        self._dataset_cache_dir = dataset.cache_dir
        self.psid_eos_token = self.eos_token
        copref_offset = self.psid_eos_token + 1
        self.copref_token_ids = [
            copref_offset + level for level in range(self.n_digit)
        ]
        self.level_offsets = [1]
        for digit in range(1, self.n_digit):
            self.level_offsets.append(self.level_offsets[-1] + self.codebook_sizes[digit - 1])
        self.collate_fn = {
    ...
    def vocab_size(self) -> int:
        return self.psid_eos_token + 1 + self.n_digit
    ...
    def tokenize(self, datasets: dict) -> dict:
        tokenized_datasets = {}
        cache_root = os.path.join(self._dataset_cache_dir, 'processed', 'colagr_tokenized_v2')
        ...
        for split in datasets:
            ...
            tokenized_datasets[split] = datasets[split].map(
                ...
                cache_file_name=os.path.join(cache_root, f'{split}.arrow'),
        ...
        for split in datasets:
            ...
            tokenized_datasets[split].set_format(
                ...
                # Keep the hot-path batches tensor-only.  target_item is not
                # consumed by CoLaGR training/evaluation and otherwise gets
                # materialized and carried through every DataLoader worker.
    ...
    def collate_fn_train(self, batch: list) -> dict:
        return self.collate_fn_common(batch)
