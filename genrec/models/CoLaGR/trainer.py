...
COPREF_TENSOR_FORMATS = {
    'colagr_copref_tensor_v1',
    'colagr_copref_prefix_tensor_v2',
}
...
class CoLaGRTrainer(Trainer):
    def __init__(self, config, model, tokenizer):
        ...
        if self._config_bool('use_coreason', False):
            raise ValueError(
                'CoReason is not part of this release. Use the documented '
                'stepwise CoPref and CoLeaf variants instead.'
            )
        self.num_levels = tokenizer.n_digit
        self.tokenizer = tokenizer
        load_eval_copref_diagnostics = bool(config.get('load_eval_copref_diagnostics', False))
        self.coreason_target_source = str(
            config.get('coreason_target_source', 'future_value')
        ).strip().lower()
        if self.coreason_target_source not in {'future_value', 'copref'}:
            raise ValueError(
                'coreason_target_source must be either future_value or copref.'
            )
        ...
        self.train_copref_enabled = (
            self._config_bool('use_copref_loss', True)
        ) or (
            self._config_bool('use_coreason', False)
            and float(config.get('lambda_reason', 0.0)) > 0.0
        )
        ...
        self.coreason_targets_by_split = {
            'train': self._load_coreason_targets(config.get('coreason_train_path'))
            if self._config_bool('use_coreason', False)
            and float(config.get('lambda_reason', 0.0)) > 0.0
            and self.coreason_target_source == 'future_value' else None,
            'val': None,
            'test': None,
        }
        self._preload_copref_to_device()
    ...
    def _load_copref(self, path):
        data = torch.load(path, map_location='cpu') if path is not None else None
        if isinstance(data, dict) and data.get('format') in COPREF_TENSOR_FORMATS:
            sample_ids = data['sample_id'].long()
            direct = (
                sample_ids.numel() > 0
                and int(sample_ids.min()) == 0
        ...
        if data is not None:
            raise ValueError(
                f'Unsupported CoPref asset in {path}: '
                f'{data.get("format") if isinstance(data, dict) else type(data).__name__}. '
                'Expected a supported CoPref tensor format: '
                f'{sorted(COPREF_TENSOR_FORMATS)}.'
            )
        return None

    def _load_coreason_targets(self, path):
        return self._load_level_teacher(
            path,
            'colagr_coreason_value_tensor_v1',
            'coreason_targets',
        )

    def _preload_copref_to_device(self):
        """Move tensor-format CoPref once instead of copying every batch."""
        if not self._config_bool('preload_copref_to_device', False):
            return
        for data in self.copref_by_split.values():
            if not (
                isinstance(data, dict)
                and data.get('format') in COPREF_TENSOR_FORMATS
            ):
                continue
            data['copref'] = [
                level_teacher.to(self.accelerator.device, non_blocking=True)
                for level_teacher in data['copref']
            ]
            data['_device_preloaded'] = True
        for data in self.coreason_targets_by_split.values():
            if not (isinstance(data, dict) and data.get('format') == 'colagr_coreason_value_tensor_v1'):
                continue
            data['coreason_targets'] = [
                level_teacher.to(self.accelerator.device, non_blocking=True)
                for level_teacher in data['coreason_targets']
            ]
            data['_device_preloaded'] = True

    def _move_tensor_batch(self, batch):
        return {
            # sample_id is only used for CPU-side teacher lookup. Moving it to
            # CUDA and immediately copying it back forced a synchronization.
            key: value.to(self.accelerator.device, non_blocking=True)
            if torch.is_tensor(value) and key != 'sample_id' else value
    ...
    def get_batch_coreason_targets(self, sample_ids, split):
        data = self.coreason_targets_by_split.get(split)
        return self._get_batch_level_teacher(data, sample_ids, 'coreason_targets')
    ...
    def _get_batch_level_teacher(self, data, sample_ids, value_key):
        ...
        if isinstance(data, dict) and data.get('format') in {
            *COPREF_TENSOR_FORMATS,
            ...
            if data.get('_device_preloaded', False):
                # Preloaded tensors still follow the asset's sample_id order.
                # Only contiguous 0..N-1 assets can use sample_id directly;
                # prefix-filtered CoReason assets commonly require mapping.
                cpu_sample_ids = sample_ids.detach().cpu().long()
                if data.get('_direct_sample_id_index', False):
                    row_indices = cpu_sample_ids.to(
                ...
                return [
                    level_teacher.index_select(0, row_indices)
    ...
    def _attach_coprefs(self, batch, split):
        if split == 'train' and not self.train_copref_enabled:
            return batch
        ...
        if split == 'train':
            if (
                self._config_bool('use_coreason', False)
                and float(self.config.get('lambda_reason', 0.0)) > 0.0
                and self.coreason_target_source == 'copref'
            ):
                if coprefs is None:
                    raise ValueError(
                        'coreason_target_source=copref requires train CoPref targets.'
                    )
                batch['coreason_targets'] = coprefs
            else:
                coreason_targets = self.get_batch_coreason_targets(batch['sample_id'], split)
                if coreason_targets is not None:
                    batch['coreason_targets'] = coreason_targets
        return batch
    ...
    def fit(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            total_loss_gen = torch.zeros((), device=device)
            total_loss_pref = torch.zeros((), device=device)
            total_loss_reason = torch.zeros((), device=device)
            pref_sums = torch.zeros(self.num_levels, dtype=torch.float, device=device)
            pref_counts = torch.zeros(self.num_levels, dtype=torch.float, device=device)
            ...
            for batch in progress_bar:
                ...
                total_loss_gen += outputs.loss_gen.detach()
                total_loss_pref += outputs.loss_pref.detach()
                total_loss_reason += outputs.loss_reason.detach()
                for level, value in enumerate(getattr(outputs, 'per_level_pref', [])):
                    pref_sums[level] += value.detach()
                    pref_counts[level] += 1.0

            denom = max(len(train_dataloader), 1)
            self.accelerator.log({
                'Loss/train_loss': total_loss.item() / denom,
                'Loss/loss_gen': total_loss_gen.item() / denom,
                'Loss/loss_pref': total_loss_pref.item() / denom,
                'Loss/loss_reason': total_loss_reason.item() / denom,
            }, step=epoch + 1)
            pref_means = (pref_sums / pref_counts.clamp_min(1)).tolist()
            log_message = (
                f'[Epoch {epoch + 1}] Train Loss: {total_loss.item() / denom}; '
                f'gen={total_loss_gen.item() / denom}; pref={total_loss_pref.item() / denom}; '
                f'reason={total_loss_reason.item() / denom}; '
