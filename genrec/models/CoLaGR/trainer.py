...
COPREF_TENSOR_FORMATS = {
    'colagr_copref_global_tensor_v1',
...
class CoLaGRTrainer(Trainer):
        ...
        if isinstance(data, dict) and data.get('format') in {
            *COPREF_TENSOR_FORMATS,
            ...
            if data.get('_device_preloaded', False):
                ...
                # Prefix-filtered CoPref assets commonly require mapping.
    ...
    def fit(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            denom = max(
                min(len(train_dataloader), int(max_train_batches))
                if max_train_batches is not None else len(train_dataloader),
                1,
            )
    ...
    def evaluate(self, dataloader, split='test'):
        ...
        max_eval_batches = self.config.get('max_eval_batches')
        for batch_index, batch in enumerate(progress_bar):
            if max_eval_batches is not None and batch_index >= int(max_eval_batches):
                break
            with torch.no_grad():
                batch = self._move_tensor_batch(batch)
