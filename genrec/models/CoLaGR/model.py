...
class CoLaGR(AbstractModel):
    """
    CoLaGR keeps conflict-free semantic IDs as labels. The mainline uses CoPref
    ...
    def __init__(self, config, dataset, tokenizer):
        ...
        if self.copref_reasoning_mode not in {'per_level', 'centralized', 'one_shot'}:
            raise ValueError(
                "copref_reasoning_mode must be 'per_level', 'centralized', or 'one_shot'."
            )
        ...
        self.copref_token_ids = torch.tensor(tokenizer.copref_token_ids, dtype=torch.long)
    ...
    def _prepare_checkpoint_state_dict(self, state_dict):
        ...
        for key, value in state_dict.items():
            # Formal checkpoints predating the Global-only cleanup may retain
            # this unused head. It never participates in current inference.
            if key.startswith('coreason_heads.'):
                continue
    ...
    def build_decoder_inputs(self, labels_sid):
        ...
        if not self.use_copref_module:
            pref_positions = None
        elif self.copref_reasoning_mode == 'centralized':
            # Preserve the per-level CP/SID layout and all three targets, but
            # read every target from CP1 to isolate supervision placement.
            pref_positions = action_positions[0].expand(self.num_levels)
        else:
    ...
    def forward(self, batch):
        ...
        loss = loss_gen + lambda_pref * loss_pref
    ...
    def _next_level_log_probs(self, encoder_outputs, attention_mask, decoder_ids, level, prefixes=None):
        ...
        pref_hidden = None
        if self.use_copref_module:
            pref_hidden = decoder_outputs.last_hidden_state[:, -1, :]
    ...
    def generate_colagr_beam(
        ...
        for level in range(self.num_levels):
            ...
            if return_trace:
                beam_trace.append({
                    'sid_tokens': sid_tokens.detach().clone(),
                    'scores': beam_scores.detach().clone(),
                })
        ...
        result = sid_tokens[:, :n_return_sequences, :]
        if return_trace and return_scores:
            return result, beam_scores[:, :n_return_sequences], beam_trace
        if return_trace:
            return result, beam_trace
        if return_scores:
            return result, beam_scores[:, :n_return_sequences]
        return result

    def generate(self, batch, n_return_sequences=1, return_scores=False, return_trace=False):
        self._assert_teacher_free_generation(batch)
        with torch.no_grad():
            if int(self.config.get('num_beams', 1)) == 1:
                if return_scores or return_trace:
                    raise ValueError('return_scores and return_trace require beam search (num_beams > 1).')
                return self.generate_colagr_greedy(batch, n_return_sequences)
            return self.generate_colagr_beam(
                batch,
                n_return_sequences,
                return_scores,
                return_trace,
