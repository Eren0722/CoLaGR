...
class PSID(AbstractModel):
    """
    PSID (Purely Semantic ID) model for generative recommendation.
    
    PSID ensures all semantic IDs are unique (non-conflicting) while maintaining purely 
    semantic tokens. Unlike approaches that add non-semantic tokens to resolve conflicts,
    PSID reassigns conflicting items to nearby semantic IDs, trading some compression 
    accuracy for conflict-free purely semantic representations.
    
    Based on the generative retrieval framework from:
    ...
    def __init__(
        ...
        super(PSID, self).__init__(config, dataset, tokenizer)
